...
export const SystemStatus = ({ user, currentApp, nodeCount, onAuth, onToggleNav, notifications = [] }) => {
  ...
  return (
    <div 
      className={`system-status-readout ${showRoles ? 'is-connected' : 'is-idle'} ${isAuth ? 'is-auth' : ''} step-${mobileStep}`}
    ...
    >
      ...
      <SystemPower isAuth={isAuth} showRoles={showRoles} onPower={handlePowerBtn}
        onIdentity={handleIdentity} onClose={() => setShowRoles(false)} search={search} setSearch={setSearch} />
    </div>
  );
};
