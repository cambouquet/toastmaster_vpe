...
import { GuestBroadcast } from './GuestBroadcast';
...
export const LiveToolkit = ({ state, onAction }) => {
  ...
  const setRunning = (v) => { if (isGuest) return; onAction(v ? 'TIMER_START' : 'TIMER_STOP', v ? current.member : { name: current.member, time: format(time) }); rawSR(v); };
  ...
  return (
    <div className={`card live-toolkit ${isGuest ? 'guest-mode' : ''} ${running && isGlobal ? 'pulse-border' : ''} light-${isGlobal ? getLight() : ''}`}>
      <ToolkitHeader setLocalIdx={setLocalIdx} localIdx={localIdx} segments={segments} current={current} running={running} isGlobal={isGlobal} />
      {!isGuest ? (
        <div className="tool-grid">
          ...
          <div className={`eval-vessel ${running && isGlobal ? 'locked' : 'ready'}`}><LiveEval activeIdx={activeIdx} state={state} onAction={onAction} /></div>
        </div>
      ) : <GuestBroadcast format={format} isGlobal={isGlobal} time={time} getTarget={getTarget} />}
    </div>
  );
};
