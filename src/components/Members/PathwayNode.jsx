  ...
  return (
    <div className="enrolled-item" onMouseLeave={() => { setPick(false); setConf(false); }}>
      <div className="p-header">
        {conf ? <span className="p-name confirm" onClick={handleDel}>REMOVE PATHWAY?</span> : (
          <div className={`p-selector ${pick ? 'open' : ''}`}>
            <span className="p-name-val" onClick={() => setPick(!pick)}>{item.name}</span>
        ...
        <button className="p-del-btn" onClick={handleDel}><svg viewBox="0 0 24 24" width="10"><path fill="currentColor" d="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19V4M6,19A2,2,0,0,0,8,21H16A2,2,0,0,0,18,19V7H6V19Z" /></svg></button>
      </div>
